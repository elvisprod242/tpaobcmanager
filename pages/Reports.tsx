...
import { Rapport, Partenaire, Invariant, Infraction, UserRole, Conducteur, Vehicule, CleObc } from '../types';
...
export const Reports = ({ selectedPartnerId, partners, invariants, reportsData, setReportsData, infractionsData, setInfractionsData, userRole, drivers, vehicles, keys }: ReportsProps) => {
    ...
    const handleSaveInfraction = () => {
        ...
        alert("Infraction créée avec succès.");
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file) return;
        alert("Simulation : Fichier importé avec succès.");
    };
    ...
    const filteredReports = useMemo(() => {
         return reports.filter(r => {
            ...
            const driver = drivers.find(c => c.id === r.conducteur_id);
    ...
    }, [reports, filter, dateFilter, startDate, endDate, selectedPartnerId, activeTab, invariants, drivers]);

    useEffect(() => {
        setCurrentPage(1);
        setSelectedReports(new Set());
    }, [filter, dateFilter, startDate, endDate, selectedPartnerId, itemsPerPage, activeTab]);
    ...
    const availableDrivers = useMemo(() => {
        ...
        return drivers.filter(c => isDriverLinkedToPartner(c, formData.partenaire_id!, keys));
    }, [formData.partenaire_id, drivers, keys]);

    const availableVehicles = useMemo(() => {
        if (!formData.partenaire_id) return [];
        return vehicles.filter(v => v.partenaire_id === formData.partenaire_id);
    }, [formData.partenaire_id, vehicles]);
    ...
    const getViewingDetails = () => {
        ...
        const driver = drivers.find(c => c.id === viewingReport.conducteur_id);
        const vehicle = vehicles.find(v => v.id === viewingReport.vehicule_id);
        const invariant = invariants.find(i => i.id === viewingReport.invariant_id);
    ...
    return (
        <div className="space-y-6 animate-fade-in w-full pb-8">
            ...
            {/* Modales ( inchangées sauf pour la gestion des props ) */}
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Modifier Rapport' : 'Nouveau Rapport'} size="large" footer={<><button onClick={() => setIsModalOpen(false)} className="flex-1 px-4 py-2.5 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 font-semibold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">Annuler</button><button onClick={handleSave} className="flex-1 px-4 py-2.5 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-900/20 transition-all flex items-center justify-center gap-2"><Save size={18} /> Enregistrer</button></>}>
                <div className="space-y-6">
                    <div className="p-4 bg-slate-50 dark:bg-slate-900/30 rounded-xl border border-slate-100 dark:border-slate-700">
                        <h4 className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider mb-4 flex items-center gap-2"><Users size={14} /> Contexte du rapport</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <FormInput label="Date" type="date" value={formData.date} onChange={(e: any) => setFormData({...formData, date: e.target.value})} />
                            <FormSelect label="Partenaire" value={formData.partenaire_id} onChange={(e:any) => setFormData({...formData, partenaire_id: e.target.value, conducteur_id: '', vehicule_id: ''})} options={[{value: '', label: 'Sélectionner...'}, ...partners.map(p => ({value: p.id, label: p.nom}))]} disabled={selectedPartnerId !== 'all'} />
                            <FormSelect label="Conducteur" value={formData.conducteur_id} onChange={(e: any) => setFormData({...formData, conducteur_id: e.target.value})} disabled={!formData.partenaire_id} options={[{ value: '', label: formData.partenaire_id ? 'Sélectionner un conducteur...' : 'Aucun partenaire actif' }, ...availableDrivers.map(d => ({ value: d.id, label: `${d.prenom} ${d.nom}` }))]} />
                            <FormSelect label="Véhicule" value={formData.vehicule_id} onChange={(e: any) => setFormData({...formData, vehicule_id: e.target.value})} disabled={!formData.partenaire_id} options={[{ value: '', label: 'Sélectionner un véhicule...' }, ...availableVehicles.map(v => ({ value: v.id, label: `${v.nom} (${v.immatriculation})` }))]} />
                        </div>
                        <div className="mt-4"><FormSelect label="Invariant (Règle associée)" value={formData.invariant_id} onChange={(e: any) => setFormData({...formData, invariant_id: e.target.value})} disabled={!formData.partenaire_id} options={[{ value: '', label: 'Aucun' }, ...availableInvariants.map(i => ({ value: i.id, label: i.titre }))]} /></div>
                    </div>
                    {/* Le reste du formulaire (Horaires, Télémétrie) reste identique */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="p-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                            <h4 className="text-xs font-bold text-amber-600 dark:text-amber-400 uppercase tracking-wider mb-4 flex items-center gap-2"><Clock size={14} /> Horaires & Durées</h4>
                        ...
                        <div className="p-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                            <h4 className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider mb-4 flex items-center gap-2"><Activity size={14} /> Télémétrie</h4>
                            <div className="space-y-4">
                                <FormInput label="Distance (km)" type="number" min="0" value={formData.distance_km} onChange={(e:any) => setFormData({...formData, distance_km: parseFloat(e.target.value) || 0})} />
                                <div className="grid grid-cols-2 gap-3">
                                    <FormInput label="Vitesse Moy. (km/h)" type="number" min="0" value={formData.vitesse_moyenne} onChange={(e:any) => setFormData({...formData, vitesse_moyenne: parseFloat(e.target.value) || 0})} />
                                    <FormInput label="Vitesse Max (km/h)" type="number" min="0" value={formData.vitesse_max} onChange={(e:any) => setFormData({...formData, vitesse_max: parseFloat(e.target.value) || 0})} />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {/* Modale détail / Suppression inchangée */}
            <Modal isOpen={!!viewingReport} onClose={() => setViewingReport(null)} title="Détails du Rapport" size="large" footer={<div className="flex gap-2 w-full">{!isReadOnly && (<><button onClick={handleOpenInfractionForm} className="flex-1 px-4 py-2 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 font-semibold rounded-lg hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors flex items-center justify-center gap-2 border border-red-200 dark:border-red-900"><AlertTriangle size={16} /> Signaler Infraction</button><button onClick={() => requestDelete(viewingReport!.id)} className="flex-1 px-4 py-2 bg-slate-50 dark:bg-slate-800 text-red-600 dark:text-red-400 font-semibold rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors flex items-center justify-center gap-2 border border-slate-200 dark:border-slate-700 hover:border-red-200"><Trash2 size={16} /> Supprimer</button></>)}<button onClick={() => setViewingReport(null)} className={`px-6 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors font-medium ${isReadOnly ? 'w-full' : ''}`}>Fermer</button></div>}>
                {viewingReport && (() => {
                        ...
                        <div className="space-y-6">
                            <div className="flex flex-col md:flex-row items-center gap-6 p-6 bg-slate-50 dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-700">
                                <div className="flex items-center gap-4 flex-1">
                                    <div className="w-16 h-16 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-700 dark:text-blue-300 font-bold text-xl shadow-inner">{details?.driver ? `${details.driver.prenom[0]}${details.driver.nom[0]}` : <Users />}</div>
                                    <div>
                                        <h3 className="text-lg font-bold text-slate-800 dark:text-white">{details?.driver ? `${details.driver.prenom} ${details.driver.nom}` : 'Conducteur Inconnu'}</h3>
                                        <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mt-1"><Calendar size={14} /><span>{new Date(viewingReport.date).toLocaleDateString('fr-FR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span></div>
                                    </div>
                                </div>
                                <div className="flex flex-col items-end gap-2">
                                    <div className="px-3 py-1 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-sm font-medium flex items-center gap-2 shadow-sm"><Truck size={14} className="text-slate-400" />{details?.vehicle ? details.vehicle.nom : 'Véhicule N/A'}<span className="text-slate-400">|</span><span className="font-mono text-slate-600 dark:text-slate-300">{details?.vehicle?.immatriculation || '---'}</span></div>
                                    {details?.invariant && <div className="px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 text-xs font-bold border border-blue-100 dark:border-blue-900 flex items-center gap-1"><ShieldCheck size={12} /> {details.invariant.titre}</div>}
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="p-4 rounded-xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col items-center justify-center text-center gap-2"><div className="p-2 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 rounded-lg mb-1"><MapPin size={24} /></div><span className="text-xs font-bold text-slate-400 uppercase tracking-wide">Distance</span><span className="text-2xl font-bold text-slate-800 dark:text-white">{viewingReport.distance_km} km</span></div>
                                <div className="p-4 rounded-xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col items-center justify-center text-center gap-2"><div className="p-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 rounded-lg mb-1"><Activity size={24} /></div><span className="text-xs font-bold text-slate-400 uppercase tracking-wide">Vitesse Moy.</span><span className="text-2xl font-bold text-slate-800 dark:text-white">{viewingReport.vitesse_moyenne} km/h</span></div>
                                <div className="p-4 rounded-xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col items-center justify-center text-center gap-2"><div className="p-2 bg-amber-50 dark:bg-amber-900/20 text-amber-600 rounded-lg mb-1"><Gauge size={24} /></div><span className="text-xs font-bold text-slate-400 uppercase tracking-wide">Vitesse Max</span><span className="text-2xl font-bold text-slate-800 dark:text-white">{viewingReport.vitesse_max} km/h</span></div>
                            </div>
                            <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                                <h4 className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-4 flex items-center gap-2"><Clock size={16} /> Chronologie</h4>
                                <div className="flex items-center justify-between text-sm mb-2"><span className="font-mono text-slate-500">{viewingReport.heure_debut}</span><div className="flex-1 mx-4 h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden relative"><div className="absolute left-0 top-0 bottom-0 bg-blue-500 w-3/4 rounded-full opacity-30"></div></div><span className="font-mono text-slate-500">{viewingReport.heure_fin}</span></div>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-6">
                                    <div className="bg-amber-50 dark:bg-amber-900/10 p-3 rounded-lg border-l-4 border-amber-500"><span className="text-xs text-slate-500 block">Conduite</span><span className="font-bold text-slate-800 dark:text-white font-mono">{viewingReport.temps_conduite}</span></div>
                                    <div className="bg-emerald-50 dark:bg-emerald-900/10 p-3 rounded-lg border-l-4 border-emerald-500"><span className="text-xs text-slate-500 block">Attente</span><span className="font-bold text-slate-800 dark:text-white font-mono">{viewingReport.temps_attente}</span></div>
                                    <div className="bg-slate-50 dark:bg-slate-900/30 p-3 rounded-lg border-l-4 border-slate-400"><span className="text-xs text-slate-500 block">Ralenti</span><span className="font-bold text-slate-800 dark:text-white font-mono">{viewingReport.duree_ralenti}</span></div>
                                    <div className="bg-blue-50 dark:bg-blue-900/10 p-3 rounded-lg border-l-4 border-blue-500"><span className="text-xs text-slate-500 block">Durée Totale</span><span className="font-bold text-slate-800 dark:text-white font-mono">{viewingReport.duree}</span></div>
                                </div>
                            </div>
                        </div>
                    );
            ...
            <Modal isOpen={!!deleteAction} onClose={() => setDeleteAction(null)} title="Confirmation de suppression" size="default" footer={<><button onClick={() => setDeleteAction(null)} className="flex-1 px-4 py-2.5 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl font-semibold transition-colors">Annuler</button><button onClick={confirmDelete} className="flex-1 px-4 py-2.5 bg-red-600 text-white rounded-xl hover:bg-red-700 font-semibold shadow-lg shadow-red-900/20 transition-all flex items-center justify-center gap-2"><Trash2 size={18} /> Confirmer</button></>}>
                <div className="flex flex-col items-center text-center p-6 space-y-4">
                    ...
                    <div className="space-y-2"><h3 className="text-lg font-bold text-slate-800 dark:text-white">Êtes-vous sûr de vouloir supprimer ?</h3><p className="text-slate-500 dark:text-slate-400 max-w-sm mx-auto">{deleteAction?.type === 'bulk' ? `Vous allez supprimer définitivement ${selectedReports.size} rapport(s).` : "Ce rapport sera définitivement effacé de la base de données."}</p><p className="text-xs text-red-500 font-medium pt-2">Cette action est irréversible.</p></div>
                </div>
            </Modal>
        </div>
    );
