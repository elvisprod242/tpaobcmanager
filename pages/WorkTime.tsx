...
// Helper précis avec secondes pour les totaux si besoin, ici on garde le format HHhMM
const secondsToFullTimeString = (totalSeconds: number): string => {
    const h = Math.floor(totalSeconds / 3600);
    const m = Math.floor((totalSeconds % 3600) / 60);
...
export const WorkTime = ({ selectedPartnerId, partners, globalYear, analyses, setAnalyses }: WorkTimeProps) => {
    ...
    const getExportData = () => {
        ...
        const data = filteredReports.map(r => {
            ...
            const serviceSec = timeStringToSeconds(r.temps_conduite) + timeStringToSeconds(r.temps_attente);
            return {
                Date: new Date(r.date).toLocaleDateString('fr-FR'),
                Jour: r.jour,
                Début: r.heure_debut,
                Fin: r.heure_fin,
                Service: secondsToFullTimeString(serviceSec),
    ...
    const handleExportExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Temps de Travail");
        
        XLSX.writeFile(workbook, `Temps_Travail_${driverName}_${monthName}_${currentYear}.xlsx`);
    };
    ...
    const handleExportPDF = () => {
        ...
        doc.text("Rapport Temps de Travail", 14, 20);
        ...
        doc.text(monthlyStats.service, 125, 40);

        // --- Tableau ---
        const tableColumn = ["Date", "Jour", "Début", "Fin", "Service", "Analyse", "Action"];
        ...
        autoTable(doc, {
            ...
            columnStyles: {
                ...
                4: { cellWidth: 20, halign: 'center', fontStyle: 'bold', textColor: [37, 99, 235] }, // Service (Blue)
        ...
        doc.save(`Rapport_${driverName}_${monthName}.pdf`);
    };
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-8 print:p-0 print:space-y-2">
                ...
                {selectedDriverId && (
                    <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto">
                        <div className="flex gap-4 flex-1">
                             <div className="bg-blue-50 dark:bg-blue-900/20 px-4 py-2 rounded-xl border border-blue-100 dark:border-blue-900/50 flex-1 md:flex-none min-w-[120px]">
            ...
            <div className="hidden print-only mb-6">
                <h1 className="text-2xl font-bold mb-2">Rapport Temps de Travail</h1>
                ...
                <div className="mt-4 border p-4 flex gap-8">
                     <div>Total Service: <strong>{monthlyStats.service}</strong></div>
                </div>
            </div>
            ...
            {!selectedDriverId ? (
                <div className="text-center py-20 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 no-print">
                    ...
                    <p className="text-slate-500 font-medium">Veuillez sélectionner un conducteur pour voir son temps de travail.</p>
                </div>
                ...
                <div className="space-y-8">
                    {Object.entries(reportsByWeek).map(([week, data]) => {
                        ...
                        // Change: Service = Conduite + Attente
                        const weekServiceSec = weekReports.reduce((acc, r) => acc + timeStringToSeconds(r.temps_conduite) + timeStringToSeconds(r.temps_attente), 0);
                            ...
                            <div key={week} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden break-inside-avoid">
                                ...
                                <div className="bg-slate-50 dark:bg-slate-900/50 px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center print:bg-gray-100 print:border-gray-300">
                                    ...
                                    <div className="flex gap-4 text-sm">
                                        <span className="text-slate-500 dark:text-slate-400">Total Service: <span className="font-bold text-slate-800 dark:text-white">{secondsToTimeString(weekServiceSec)}</span></span>
                                    </div>
                                </div>
                                ...
                                <div className="divide-y divide-slate-100 dark:divide-slate-700 print:divide-gray-200">
                                    {weekReports.map(report => {
                                        ...
                                        // Change: Calculate service time for display
                                        const serviceSec = timeStringToSeconds(report.temps_conduite) + timeStringToSeconds(report.temps_attente);
                                        const serviceTimeDisplay = secondsToFullTimeString(serviceSec);
                                            ...
                                            <div key={report.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors group print:hover:bg-transparent">
                                                <div className="flex flex-col md:flex-row md:items-center gap-4 justify-between">
                                                    ...
                                                    <div className="flex-1 min-w-0 grid grid-cols-2 md:grid-cols-3 gap-4 items-center">
                                                        ...
                                                        <div className="text-sm">
                                                            <span className="block text-xs text-slate-400">Total Service</span>
                                                            {/* Display Calculated Service Time */}
                                                            <span className="font-mono font-bold text-blue-600 dark:text-blue-400">{serviceTimeDisplay}</span>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="md:w-1/3 flex justify-end">
                                                        {analysis ? (
                                                            <div className="relative w-full bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-lg p-3 text-sm cursor-pointer hover:bg-red-100 dark:hover:bg-red-900/20 transition-colors group/analysis print:bg-transparent print:border-gray-300" onClick={() => handleOpenAnalysisModal(report)}>
                                                                ...
                                                                <div className="flex items-start gap-2">
                                                                    ...
                                                                    <div>
                                                                        <p className="font-bold text-red-800 dark:text-red-300 text-xs uppercase mb-1">Analyse : {analysis.analyse_cause}</p>
                ...
                <div className="space-y-4">
                    <div className="space-y-2">
                        ...
                        <textarea 
                            ...
                            placeholder="Décrivez la cause de l'anomalie ou du dépassement..."
                            autoFocus
                        />
                    </div>
                    ...
                    <FormInput 
                        ...
                        placeholder="Ex: Optimisation planning, Formation..."
                    />
                </div>
            </Modal>
        </div>
